...
export default async function AccessPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string; invite?: string }> }) {
  ...
  return <>
    <PageHeading section="Administration" title="Account Access" description="Review who can use the private workspace, choose their roles, and connect players to the correct athlete profile."><Link href="/admin/access/status" className="btn btn-secondary">Invitation Status</Link><Link href="/admin/access/bulk" className="btn btn-primary">Invite Remaining Players</Link></PageHeading>
