...
export default async function PerformanceImport({ searchParams }: { searchParams: Promise<{error?:string;import?:string}> }) {
  ...
  return <>
    <PageHeading section="Information Imports" title="Shared Measurements" description="Publish reviewed readings to the private player profiles."><Link href="/preview/import" className="btn btn-secondary">Browser Import Center</Link></PageHeading>
    ...
    <h2 className="mb-4 mt-8 text-lg font-bold">{roles.includes("admin") ? "Recent Shared Imports" : "Your Recent Shared Imports"}</h2>
