...
export default async function ImportsPage() {
  ...
  return <>
    ...
    <details className={styles.otherTools}><summary className="cursor-pointer text-sm font-semibold">Import History &amp; Other Tools</summary><div className="mt-4 flex flex-wrap gap-3"><Link href="/imports/movement" className="btn btn-secondary">Movement Screenings</Link><Link href="/admin/performance" className="btn btn-secondary">Import History</Link><Link href="/preview/import" className="btn btn-secondary">Advanced File Import</Link>{roles.includes("admin") && <><Link href="/admin/import" className="btn btn-secondary">Roster Import</Link><Link href="/admin/import/renpho" className="btn btn-secondary">Roster RENPHO IDs</Link></>}</div></details>
  </>;
}
