...
export default async function GameStatsPage(){
  ...
  return <><PageHeading section="Pacific Baseball / Competition" title="Game Stats" description="Fall 2026 · Recorded QPA and pitching statistics from the team sheets.">{staff&&<Link href="/imports/game-stats" className="btn btn-secondary">Review Game Snapshot</Link>}</PageHeading>{!ids.length?<AthleteGameStats stats={[]}/>:ids.map(id=><div key={id}><h2 className="mt-6 text-xl font-bold"><Link href={`/athletes/${id}`}>{names.get(id)??"Athlete"}</Link></h2><AthleteGameStats stats={stats.filter(row=>row.athlete_id===id)} comparisons={comparisons.get(id)}/></div>)}</>;
}
