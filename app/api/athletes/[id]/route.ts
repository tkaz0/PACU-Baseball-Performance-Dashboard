...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!access) return Response.json({ error: "Access denied" }, { status: reason === "configuration" ? 503 : reason === "forbidden" || reason === "preview" ? 403 : 401 });
  const { id } = await params;
  if (!UUID_PATTERN.test(id) || !canReadPresentedAthlete(access, id)) return Response.json({ error: "Not found" }, { status: 404 });
