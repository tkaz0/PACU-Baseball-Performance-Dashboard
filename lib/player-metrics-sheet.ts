...
const columns: Record<number,string> = {3:"Boxer T",4:"Boxer T",5:"Boxer T",6:"Home to First",7:"Home to Second",8:"12 ft Steal Start",9:"12 ft Steal Start",10:"12 ft Steal Start",11:"Steal Reaction",12:"Steal Reaction",13:"Steal Reaction",14:"Steal Start · 12–42 ft",15:"Steal Start · 12–42 ft",16:"Steal Start · 12–42 ft"};
type Input = { rows: unknown[][]; sheet: string; measuredAt: string; fileHash: string; identities: ReadonlyMap<string,string> };
/** Explicitly reviewed dates and identities only. Raw numeric trials, never cached summaries. */
export function preparePlayerMetricsTrials({rows,sheet,measuredAt,fileHash,identities}:Input) {
  ...
  for(let index=1;index<rows.length;index++) {
    ...
    for(const [column,metric] of trials) {
      ...
      if(typeof value!=="number"||!Number.isFinite(value)||value<=0)throw new Error(`Row ${index+1}, column ${Number(column)+1}: review the recorded time. Blanks are allowed; zero, text and formulas are not trials.`);
      measurements.push({id:`observation:${JSON.stringify([fileHash,sheet,index+1,Number(column)])}`,athlete_code:code,metric,value,unit:"s",measured_at:measuredAt,source:"Player Metrics",source_file:"Player Metrics 2026-27.xlsx",source_sheet:sheet,source_row:index+1,file_hash:fileHash});
    }
  }
