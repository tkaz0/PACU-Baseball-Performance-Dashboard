...
const metrics:Record<string,string>={pa:"Game PA",ab:"Game AB",base_hit:"Game Hits",pumps:"Game HR",rbi:"Game RBI",sb:"Game SB",gdp:"Game GDP",qpa_pct:"Game QPA %"};
...
export function qpaAnalytics(stats:readonly SharedGameStat[]):AnalyticsReading[]{
 ...
 for(const [athleteId,rows] of groups){if(new Set(rows.map(r=>r.snapshot_id)).size!==1)continue;
  ...
  for(const rate of battingRates(rows))result.push({...base,id:JSON.stringify([first.snapshot_id,athleteId,rate.metric]),metric:rate.metric,label:`Game ${rate.label}`,unit:rate.unit,value:rate.value});
 }
 return result;
}
