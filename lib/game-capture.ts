...
export function validateGameMappings(input:unknown,contract:ReviewedGameSource,now=Date.now()):{identities:ReviewedGameIdentity[];events:ReviewedPitchingEvent[]}{
 ...
 const identities=input.identities.map(m=>{if(!object(m)||typeof m.sourceName!=="string"||!m.sourceName.trim()||m.sourceName.length>300||typeof m.athleteCode!=="string"||m.athleteCode.length>13||!(m.athleteCode==="exclude"||/^PAC-[0-9]{4,9}$/.test(m.athleteCode)))return fail();const key=m.sourceName.trim().replace(/\s+/g," ").toLowerCase();if(seen.has(key))return fail();seen.add(key);return{sourceName:m.sourceName,athleteCode:m.athleteCode};});
 const events=input.events.map(e=>{if(!object(e)||!Number.isSafeInteger(e.headerRow)||!Number.isSafeInteger(e.firstRow)||!Number.isSafeInteger(e.lastRow)||typeof e.eventId!=="string"||e.eventId.length>80||(e.playedOn!==null&&typeof e.playedOn!=="string"))return fail();return{headerRow:e.headerRow as number,firstRow:e.firstRow as number,lastRow:e.lastRow as number,eventId:e.eventId,playedOn:e.playedOn};});
