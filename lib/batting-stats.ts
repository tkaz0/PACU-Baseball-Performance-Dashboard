...
export function obpNeedsReview(rows:readonly SharedGameStat[]):boolean {
 const qpa=rows.filter(r=>r.source==="qpa_fall_2026"),v=new Map(qpa.map(r=>[r.metric,r.value]));
 return ["pa","ab","bb","hbp","sac_fly"].every(k=>v.has(k))&&v.get("ab")!+v.get("bb")!+v.get("hbp")!+v.get("sac_fly")!>v.get("pa")!;
}
