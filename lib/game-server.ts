...
export async function loadGameStats(access:Access,athleteId?:string):Promise<SharedGameStat[]> {
  ...
  return data.map(row=>{
    ...
    if(row.unit!==(rate?"%":"count")||(rate?row.value>100:!Number.isSafeInteger(row.value)||row.value>1000000000)||JSON.stringify(row.derived_from)!==JSON.stringify(rate?(qpa?[3,2]:[4,3]):row.metric==="innings_outs"?[18]:[]))return invalid();
