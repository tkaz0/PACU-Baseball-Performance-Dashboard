...
import { GAME_LEADERBOARD_METRICS, PITCHING_LEADERBOARD_METRICS, type GameComparison, type GameLeaderboardRow } from "@/lib/game-metrics";
...
export async function loadGameLeaderboards(access:Access):Promise<GameLeaderboardRow[]>{
 ...
 const seen=new Set<string>();return data.map(item=>{const r=base(item);if(!fields(r,["metric","source","eventId","playedOn","value","unit","rank","name","code","profileId","updatedAt","percentile","sampleSize",...(Object.hasOwn(r,"opportunities")?["opportunities"]:[])])||(Object.hasOwn(r,"opportunities")&&r.opportunities!==null&&(!Number.isSafeInteger(r.opportunities)||(r.opportunities as number)<=0||(r.opportunities as number)>4000000000))||(r.source==="qpa_fall_2026"?!(GAME_LEADERBOARD_METRICS as readonly string[]).includes(r.metric as string):![...PITCHING_LEADERBOARD_METRICS,"pitching_era"].includes(r.metric as string))||typeof r.unit!=="string"||r.unit!==(["pitching_k9","pitching_bb9","pitching_r9","pitching_era"].includes(r.metric as string)?"per9":["batting_avg","batting_obp"].includes(r.metric as string)?"avg":(r.metric as string).endsWith("pct")?"%":"count")||!Number.isSafeInteger(r.rank)||(r.rank as number)<1||(r.rank as number)>(r.sampleSize as number)||typeof r.name!=="string"||!r.name.trim()||r.name.length>201||/[\u0000-\u001f\u007f]/.test(r.name)||typeof r.code!=="string"||!/^PAC-\d{4,9}$/.test(r.code)||typeof r.updatedAt!=="string"||!Number.isFinite(Date.parse(r.updatedAt))||(r.profileId!==null&&(typeof r.profileId!=="string"||!UUID_PATTERN.test(r.profileId)))||(r.source==="qpa_fall_2026"?r.playedOn!==null:!(r.eventId==="fall-2026-cumulative"&&r.playedOn===null)&&!validPitchingPeriod(r.eventId,r.playedOn)))return fail();const k=JSON.stringify([r.source,r.eventId,r.metric,r.code]);if(seen.has(k))return fail();seen.add(k);return {...r,profileId:r.profileId&&canReadPresentedAthlete(access,r.profileId as string)?r.profileId:null} as GameLeaderboardRow;}).filter(r=>r.metric!=="pitching_era");
}
