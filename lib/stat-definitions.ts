...
export const STAT_DEFINITIONS: Readonly<Record<string, string>> = {
  pearson_r: "Pearson r measures the direction and strength of a straight-line relationship among the currently visible paired players. It runs from −1 (downward) to +1 (upward); near 0 means little linear relationship. At least five pairs are required here. Correlation does not show cause.",
  r_squared: "R² is Pearson r squared for this one-variable best-fit line. It runs from 0 to 1 and describes how much of the visible players’ variation in the Y variable is accounted for by the fitted line. For example, 0.64 means 64% within this sample. It is not prediction accuracy or proof of cause.",
  hitter_contact_map: "The hitter charts use reviewed Full Swing batted balls from the selected In-Game or Practice session. Only rows with paired measurements are plotted. These are recorded contacts, not confirmed hits or outcomes.",
  ev_launch_chart: "Each dot is one reviewed batted ball. Farther right means higher exit velocity in mph; higher on the chart means a higher launch angle in degrees. Dot color also follows exit velocity on a fixed display scale. The 90+ mph hard-hit and 8–32° launch-window shares use all paired balls in the selection.",
  spray_chart: "Each dot uses recorded direction and distance from the same reviewed Full Swing row. Positive direction points toward first base; the 150-foot line separates the display’s infield and outfield groups. Pull and opposite depend on the hitter’s rostered batting side. These are not confirmed hit locations or outcomes.",
  pitch_arsenal_chart: "Each mark is one staff-classified pitch type in the latest comparable session. Rightward means higher average velocity (mph); upward means higher average spin (RPM). Velocity and spin may use different numbers of recorded pitches. The chart compares recorded measurements, not pitch quality.",
  pitch_mix_chart: "Each slice shows a pitch type’s share of pitches with a saved staff classification and recorded count in this session. Unassigned or excluded pitches are outside the total, so the percentages are not shares of every pitch thrown.",
  pitching_contact_chart: "The blue and red bar divides classified contact from the Fall pitching sheet into weak (Wk) and hard (Hrd). Each percentage uses Wk + Hrd as its denominator when both counts are recorded. This is the team sheet’s classification, not the hitter chart’s 90 mph exit-velocity threshold.",
  ...
  weak_contact_pct: "Weak-contact count / (Wk + Hrd) × 100, within the same pitching period. Includes classified contact only. Higher is higher-ranked.",
  hard_contact_pct: "Hard-contact count / (Wk + Hrd) × 100, within the same pitching period. Includes classified contact only; this is the team scoring definition, not a Statcast velocity threshold. Lower is higher-ranked.",
  ...
  pitching_k9: "Strikeouts per nine innings: K × 27 / recorded outs. Inn uses baseball notation: .1 is one out and .2 is two outs. More is higher-ranked.",
  pitching_bb9: "Walks allowed per nine innings: BB × 27 / recorded outs. Uses the walks outcome column, not breaking-ball pitches. Less is higher-ranked.",
  pitching_r9: "Runs allowed per nine innings: total R × 27 / recorded outs. Includes all runs, earned or unearned. Lower is higher-ranked; this is not ERA.",
