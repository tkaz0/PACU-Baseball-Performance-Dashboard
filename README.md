...
The home route opens the private workspace for authorized accounts and the Pacific Baseball sign-in page for visitors. Every dashboard requires login. The separate [browser import workspace](https://pacubaseballperformance.com/preview) is available only to active administrators outside Coach/Player View as. Browser data becomes shared only through an administrator's explicitly approved numerical import.
...
3. Run `pnpm dev`. Access uses the project's publishable key plus the signed-in user's session. `/preview` also requires Supabase configuration and an active Admin; there is no public or development authentication bypass.
