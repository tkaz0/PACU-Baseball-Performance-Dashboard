...
it("shows a labeled HR rate with its matching power percentile",()=>{
 ...
 const html=renderToStaticMarkup(createElement(AthleteGameStats,{stats,comparisons:[{...comparison,metric:"batting_hr_pct",value:5}]}));expect(html).toContain("Home Run Rate");expect(html).toContain("5.0%");expect(html).toContain("40 PA");expect(html).toContain('aria-valuenow="75"');expect(html).toContain("doubles and triples");
