...
const fake = vi.hoisted(() => ({
  pathname: "/preview/import", state: { status: "checking", userId: null as string | null, pathname: null as string | null },
...
const html = (id = userId) => renderToStaticMarkup(AdminWorkspaceBoundary({ userId: id, children: createElement("div", null, "Private device data") }));
beforeEach(() => {
  vi.clearAllMocks(); fake.effects = []; fake.pathname = "/preview/import";
  fake.state = { status: "checking", userId: null, pathname: null };
...
describe("device data mounts only for a currently checked account and path", () => {
  it("omits local children before verification and after account or route changes", () => {
    ...
    fake.state = { status: "allowed", userId, pathname: fake.pathname };
  ...
  it("preserves the current import view during a successful periodic recheck", async () => {
    ...
    await vi.waitFor(() => expect(fake.check).toHaveBeenCalledWith(userId));
    expect(html()).toContain("Private device data");
