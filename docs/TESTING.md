...
Physicality Score validation: known directed percentile sequences, midpoint ties, competition ranks, complete-cohort minimum, missing and mismatched-date exclusions, separate sources/periods/units, deterministic partition selection, and no input mutation. The ordinary-session adapter tests role denial, provider failure and peer-link stripping; protected profile tests verify only the requested player’s score reaches rendered output. The release passes 1,477 tests across 82 files, lint, typecheck and production build. Fictional desktop/390px visual checks include the score card, rankings, missing-data state and stat popover, with no document overflow.
