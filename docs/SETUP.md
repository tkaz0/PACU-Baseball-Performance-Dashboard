...
The separate `/preview` browser import workspace requires a signed-in, active Admin or Coach outside the private Coach/Player View as. It supports fictional starter profiles and local CSV/TSV/XLSX/RENPHO review, with JavaScript, site storage and a working connection for authorization. Report processing stays on the device. Local imports do not become shared by signing in: an administrator must import the protected roster, then an Admin or Coach must approve the separate numerical sharing step. Use synthetic data in a development database first and inspect migration history before setup.
