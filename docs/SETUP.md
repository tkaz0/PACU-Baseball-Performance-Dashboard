...
The profile-tab/leaderboard upgrade adds `202609060010_profile_testing_metrics.sql` (eight explicit metric definitions and units) followed by `202609060011_team_leaderboards.sql` (narrow signed-in team comparison reads). Neither creates accounts, changes existing observations, or grants Player access to complete peer profiles. Apply only missing migrations after verifying the current schema.
