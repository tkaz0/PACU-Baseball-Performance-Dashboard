...
Height uses appended fixed metric column 21 and header source line 3001. Reopening a previously imported report can add its reviewed height without duplicating the earlier readings. Reports themselves are not stored, so missing historical heights require the original files; they cannot be recovered from saved weight or BMI.
