...
### Account setup projection
`admin_account_setup_status()` delegates to a pinned-search-path private SECURITY DEFINER function that requires a live active Admin. It returns only configured accounts with trusted roles/athlete links, contact/display labels, invitation/email-confirmation/last-sign-in timestamps and a password-present boolean. The boolean is computed inside the database from the nonempty Auth password hash; hashes, tokens and metadata never leave the function. Email confirmation establishes verified acceptance; last sign-in alone does not prove password setup. Password presence does not identify who chose it. Coach-candidate email matching supplies a display label only. No account mutations, Auth table grants, or audit writes occur. The application additionally rejects View as sessions.
