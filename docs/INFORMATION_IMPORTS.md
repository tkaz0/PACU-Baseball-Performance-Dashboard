...
The exact reviewed Full Swing Field / Live at Bat CSV now retains each batter's measured ExitSpeed and Angle together with its original pitch number and CSV row. A separate staff review shows the matched batters and all eligible pairs; the staff member confirms the match, mph, angle and Game/Intrasquad/Practice category before saving. Rows missing either value, unmatched/excluded batters and unreviewed file layouts are omitted. The 500-event limit is explicit; the importer never silently truncates. Identical source rows may be retried, while changed values, category or player assignment for a saved file/row are rejected atomically. Additional matched batters from a previously saved file keep its original date, category and display name. The contact map uses its own immutable event table and does not change summary Max/Average EV or team percentile calculations. Apply `202609220002_full_swing_contacts.sql` and `202609220003_full_swing_contact_file_consistency.sql` before deploying the consuming profile page. Only the player's own linked profile and staff can read detailed events under existing authorization; these events do not appear in team leaderboards or Analytics.

For the same exact layout, the review also carries Direction (degrees) and Distance (feet) from a batted ball's own row when both are valid. Missing or unreadable spatial pairs remain null without blocking a valid EV/launch pair. A new source row saves all available fields; an old immutable contact row may be enriched once with its original spatial readings, but existing non-null values cannot be changed or removed. Apply `202609220004_full_swing_spray.sql` before deploying this reader/importer update. Staff confirm the spatial values and category in the source-file review; positive Direction plots toward first base.
