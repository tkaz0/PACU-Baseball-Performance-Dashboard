...
Overview adds a physicality radar only when all three reviewed Fall team percentiles (total muscle mass, RENPHO body score, and body fat %) have at least five comparable athletes. It visualizes existing independent percentile ranks; lower body fat ranks higher, and the chart is not a composite score or health rating. The In-Game and Practice pitching sections show per-session reviewed pitch arsenals: average velocity versus average spin, a classified-only pitch-mix donut, and an expandable exact average/max/count table. Each session and context stays separate; missing and ambiguous readings are omitted from the affected mark, never imputed. The pitch mix denominator excludes unassigned and excluded pitches.
...
The contact section has compact sample cards and a chart switch. Percentages use all valid paired batted balls in the selected session or Fall view, even at small samples; the numerator and denominator remain visible. The owner-selected hard-hit cutoff is 90+ mph; the launch window is 8–32°, and a fourth card shows the share meeting both. These are descriptive batted-ball shares, not confirmed hit rates or performance grades. The Spray View uses the same reviewed row's Direction and Distance (feet) when both are present. Positive Direction points toward first base per the owner's export confirmation. Infield is recorded Distance under 150 feet and outfield is 150+; within each, directional percentages use ±15° for the middle lane and the rostered batting side to distinguish pull from opposite field. For switch hitters or missing batting side, the chart displays third-base/first-base sides without guessing the stance of individual swings. Missing spatial pairs stay out of spray denominators. Field lines are a visual reference, not a measured ballpark or hit outcome. It requires `202609220004_full_swing_spray.sql`. Previously saved contact rows can be enriched only by reviewing and saving the original unchanged file again.
...
September 24 profile and coach-home additions: Player result cards draw a small trend only from repeated readings with the same metric, source, unit and season; the larger testing chart remains available below. The Timeline lists saved RENPHO, athletic-testing, Blast and Full Swing report dates, grouping readings from the same file. Cumulative QPA and weekly pitching snapshots are not dated sessions. Practice vs In-Game presents latest Full Swing average bat speed and average exit velocity side by side only when recorded in the same unit. It does not compare or pool Blast with Full Swing, infer a swing count from summary observations, or calculate a Fall-wide session average.

Coach Focus is an optional staff-maintained plan on each profile. Admins and coaches may keep up to two active focus items per athlete, including a retest date and a staff-only note. A player sees only active items whose “Share this focus with player” box a coach explicitly checks, and never receives the note. Completed items remain in the staff history. Player View as also strips notes and unshared items despite the Admin's underlying database access. The staff Home “This Week” section links to new results, retests due or overdue, testing follow-ups and game rates held for source-count review. Browser-local import drafts are linked through the Import Center rather than counted as shared pending imports. Apply `202609240001_coach_focus_items.sql` and `202609240002_due_coach_focus.sql` before the consuming app.

September 25 presentation sweep: profile cards use a stronger result/label hierarchy with a compact source badge and testing-date footer. Hitting, pitching, physicality and field-throwing groups share a clear section marker, while In-Game and Practice retain explicit session headings. Card grids, composition panels and Overview highlights now respond to the profile's available width, including when the sidebar reduces the content area. Keyboard tab navigation and all role, metric, source, date, precision and percentile policies remain unchanged.
