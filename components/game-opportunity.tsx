...
export function GameOpportunity({source,metric,count}:{source:string;metric:string;count:number|null|undefined}) {
 ...
 return label&&count!=null&&count>0?<span className="mt-1 block text-xs font-normal leading-snug tracking-normal text-[var(--text-secondary)] tabular-nums">{label==="outs"?`${formatInnings(count)} IP`:`${count.toLocaleString("en-US")} ${label}`}{label!=="outs"&&<><br/><LimitedSample count={count} pitching={source==="pitching_fall_2026"}/></>}</span>:null;
}
