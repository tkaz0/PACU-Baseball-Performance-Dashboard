  ...
  return <div className="space-y-6">
    ...
    {report && <>
      <section className="panel p-5 sm:p-7">
        ...
        {shared && (matching ? <p role="status" className="muted mt-4 text-sm">Finding the player…</p> : matchingCode ? <p role="status" className="notice notice-success mt-4">Matched: <strong>{athleteName(workspace.roster.find(athlete => athlete.athlete_code === matchingCode)!)}</strong>. Confirm this is the correct player.</p> : matchPending ? <button type="button" className="btn btn-secondary mt-4" disabled={!!busy} onClick={() => { invalidate(); void matchSharedPlayer(renphoId); }}>Check RENPHO ID</button> : <p className="notice mt-4">No roster ID match yet. Choose the player for this report.</p>)}
