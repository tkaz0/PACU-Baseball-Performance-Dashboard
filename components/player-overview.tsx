...
function testingInsight(item: PlayerRelativeInsight): OverviewInsight {
  return { key: `test:${item.metric.key}`, metric: item.metric.key, label: leaderboardMetricLabel(item.metric), value: `${item.latest.value} ${item.latest.unit === "ratio" ? "" : item.latest.unit}`, percentile: item.percentile.value, sampleSize: item.percentile.sampleSize, date: item.latest.measuredAt, context: "Last Tested" };
}
function gameInsight(item: GameOverviewMetric): OverviewInsight {
  return { key: `game:${item.source}:${item.metric}`, metric: item.metric, label: item.label, value: gameValue(item.value, item.unit), percentile: item.comparison!.percentile!, sampleSize: item.comparison!.sampleSize, date: gameDate(item.updatedAt), context: item.source === "qpa_fall_2026" ? "QPA · Updated" : `Pitching ${item.playedOn} · Updated`, game: item };
}
...
function TestingComparisons({ title, cards }: { title: string; cards: readonly PlayerMetricCard[] }) {
  ...
  return <section aria-label={`${title} percentiles`} className="rounded-lg border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-4 sm:p-5"><h3 className="m-0 text-base font-bold">{title}</h3><ul className={`${styles.rows} ${styles.overviewRows}`}>{cards.map(card => {
    ...
    return <li className={styles.row} key={card.metric.key} data-overview-metric={card.metric.key}><div><h3>{leaderboardMetricLabel(card.metric)}<StatInfo metric={card.metric.key} label={leaderboardMetricLabel(card.metric)}/></h3><span className="mr-2 font-bold tabular-nums">{reading ? `${reading.value} ${reading.unit === "ratio" ? "" : reading.unit}` : "—"}</span><MeasurementChange change={playerRenphoChange(card)} metric={card.metric.key}/>{reading && <p className={styles.meta}>Last Tested: <time dateTime={reading.measuredAt}>{leaderboardTestDate(reading.measuredAt)}</time> · {reading.source}</p>}</div><div>{valid ? <><PercentileBar value={p.value} sampleSize={p.sampleSize} label={card.metric.label} descriptive={card.metric.direction === "neutral"}/><p className={styles.meta}>{p.sampleSize} comparable Pacific players{card.metric.direction === "neutral" ? " · Measured value, not a rating" : ""}</p></> : <p className={styles.meta}>{reading ? "Percentile appears after 5 comparable results." : "Not Yet Tested"}</p>}</div></li>;
  })}</ul></section>;
}
function GameComparisons({ metrics }: { metrics: GameOverviewMetric[] }) {
  if (!metrics.length) return <section aria-label="Game Stats percentiles" className="rounded-lg border border-dashed border-[var(--line-subtle)] p-4 sm:p-5"><h3 className="m-0 text-base font-bold">Game Stats</h3><p className="muted mb-0 mt-2 text-sm">Game comparisons appear after recorded Fall results are synced.</p></section>;
  return <section aria-label="Game Stats percentiles" className="rounded-lg border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-4 sm:p-5"><h3 className="m-0 text-base font-bold">Game Stats</h3><p className={styles.meta}>Fall 2026 · Counts reflect playing opportunities.</p><ul className={`${styles.rows} ${styles.overviewRows}`}>{metrics.map(item => <li className={styles.row} key={`${item.source}:${item.metric}`} data-overview-game-metric={item.metric}><div><h3>{item.label}<StatInfo metric={item.metric} label={item.label}/></h3><span className="font-bold tabular-nums">{item.metric === "batting_sb_per_pa" ? item.value.toFixed(3) : gameValue(item.value, item.unit)}</span><GameOpportunity source={item.source} metric={item.metric} count={item.opportunities}/><p className={styles.meta}>{item.source === "qpa_fall_2026" ? "QPA · Fall totals" : `Pitching · ${item.playedOn}`} · Updated {leaderboardTestDate(gameDate(item.updatedAt))}</p></div><div>{item.comparison ? <><PercentileBar value={item.comparison.percentile!} sampleSize={item.comparison.sampleSize} label={item.label} descriptive={item.direction === "neutral"}/><p className={styles.meta}>{item.comparison.sampleSize} comparable Pacific players</p></> : <p className={styles.meta}>{item.metric === "batting_sb_per_pa" ? "Recorded rate · percentile not available" : "Percentile unavailable for this result."}</p>}</div></li>)}</ul></section>;
}
...
export function PlayerOverview({ cards, gameStats = [], gameComparisons = [] }: { cards: readonly PlayerMetricCard[]; gameStats?: readonly SharedGameStat[]; gameComparisons?: readonly GameComparison[] }) {
  ...
  return <section aria-label="Player overview" className="space-y-5" data-testid="player-overview">
    ...
    <div className="grid items-start gap-4 xl:grid-cols-3">
      {[
      ...
      ].map(({ title, icon: Icon, items, note, empty }) => <section className="rounded-lg border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-4 sm:p-5" key={title} aria-label={title}>
        ...
        {items.length ? <RelativeResults items={items} /> : <p className="m-0 min-h-12 text-sm leading-6 text-[var(--text-secondary)]">{comparableCount ? empty : "Waiting for comparable testing or game results."}</p>}
      </section>)}
      <section className="rounded-lg border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-4 sm:p-5" aria-label="Biggest jumps">
    ...
    <div className="space-y-4" aria-label="Pacific percentiles"><div><h2 className="mb-2 mt-0 text-lg font-bold">Pacific Percentiles</h2><PercentileLegend/></div>
    ...
    <details className="group border-t border-[var(--line-subtle)] pt-4 text-xs text-[var(--text-secondary)]"><summary className="flex min-h-8 w-fit cursor-pointer list-none items-center gap-2 font-semibold">How This Overview Works<ChevronDown size={14} className="transition-transform group-open:rotate-180" aria-hidden="true" /></summary>
      <div className="mt-3 max-w-3xl space-y-2 leading-relaxed">
        ...
        <p>Height, weight, body composition and fastball spin stay descriptive throughout the profile. They are not labeled strengths, weaknesses or improvements. These highlights summarize current recorded results, not a prediction. Cumulative game snapshots do not establish biggest jumps.</p>
      </div>
    </details>
  </section>;
}
