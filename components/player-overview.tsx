...
function RelativeResults({ items }: { items: OverviewInsight[] }) {
  return <ul className="m-0 list-none space-y-5 p-0">{items.map(item => <li className="border-t border-[var(--line-subtle)] pt-4 first:border-0 first:pt-0" key={item.key}>
...
function TestingComparisons({ title, cards }: { title: string; cards: readonly PlayerMetricCard[] }) {
  ...
  return <section aria-label={`${title} percentiles`} className="rounded-lg border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-5 sm:p-6"><h3 className="m-0 text-base font-bold">{title}</h3><ul className={styles.rows}>{cards.map(card => {
