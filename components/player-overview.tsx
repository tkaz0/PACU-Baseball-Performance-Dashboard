...
type OverviewInsight = { key: string; metric: string; label: string; value: string; percentile: number; sampleSize: number; game?: GameOverviewMetric };
function testingInsight(item: PlayerRelativeInsight): OverviewInsight {
  return { key: `test:${item.metric.key}`, metric: item.metric.key, label: leaderboardMetricLabel(item.metric), value: `${item.latest.unit === "s" ? item.latest.value.toFixed(2) : item.latest.value} ${item.latest.unit === "ratio" ? "" : item.latest.unit}`, percentile: item.percentile.value, sampleSize: item.percentile.sampleSize };
}
function gameInsight(item: GameOverviewMetric): OverviewInsight {
  return { key: `game:${item.source}:${item.metric}`, metric: item.metric, label: item.label, value: gameValue(item.value, item.unit), percentile: item.comparison!.percentile!, sampleSize: item.comparison!.sampleSize, game: item };
}
...
function RelativeResults({ items }: { items: OverviewInsight[] }) {
  return <ul className={overview.highlightList}>{items.map(item => <li key={item.key}>
    ...
    <p className={overview.highlightMeta}>{item.game ? item.game.source === "qpa_fall_2026" ? "Hitting · " : "Pitching · " : "Testing · "}{item.sampleSize} comparable players</p>
