...
export function ImportConfirmation({ receipt }: { receipt: ImportConfirmationData }) {
  ...
  return <section ref={panel} tabIndex={-1} className={styles.receipt} aria-label="Import confirmation">
    ...
    <ul className={styles.players}>{receipt.players.map(player => <li key={`${player.code}:${player.date}`}><div><Link prefetch={false} href={player.href}>{player.name} <span aria-hidden="true">↗</span></Link><span className={styles.date}>{receipt.reportingPeriod ? `Reporting Period: ${receipt.reportingPeriod}` : `Tested ${leaderboardTestDate(player.date)}`}</span></div><p><strong>Reviewed measurements:</strong> {player.metrics.join(" · ")}</p></li>)}</ul>
