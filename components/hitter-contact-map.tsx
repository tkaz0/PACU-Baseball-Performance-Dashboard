...
export function HitterContactMap({ contacts, context, bats }: { contacts: readonly SavedContact[]; context: "in_game" | "practice"; bats?: string | null }) {
  ...
  return <section className="rounded-xl border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-4 sm:p-5" aria-label={`${context === "practice" ? "Practice" : "In-game"} hitter contact map`}>
    ...
    <p className="muted mb-0 mt-3 text-xs">Each dot is one batted ball with paired values from a reviewed Full Swing CSV row. Percentages use all paired batted balls in the selected session or Fall view. The 90+ mph hard-hit cutoff is a Pacific display choice; the launch window is 8–32°. Missing pairs, excluded players and unreviewed files are omitted. The export does not identify hit outcomes.</p>
  </section>;
}
