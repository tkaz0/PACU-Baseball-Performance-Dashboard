...
export function HitterContactMap({ contacts, context }: { contacts: readonly SavedContact[]; context: "in_game" | "practice" }) {
  ...
  return <section className="rounded-xl border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-4 sm:p-5" aria-label={`${context === "practice" ? "Practice" : "In-game"} hitter contact map`}>
    ...
    <div className="mt-4 overflow-x-auto"><svg viewBox="0 0 700 328" role="img" aria-label={`Scatter plot of ${plotted.length} batted balls with exit velocity in miles per hour on the horizontal axis and launch angle in degrees on the vertical axis`} className="min-w-[540px] w-full">
