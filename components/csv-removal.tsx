...
function FileCorrection({athleteId,item,restore=false}:{athleteId:string;item:CsvRemovalReview["active"][number]|CsvRemovalReview["archived"][number];restore?:boolean}) {
 ...
 return <section className="panel my-4 p-5"><h3 className="m-0 break-words text-base font-bold">{fullSwingFileLabel(item.sourceFile,"Full Swing · Intrasquad","firstDate" in item?item.firstDate:undefined)}</h3><p className="muted text-sm">{item.count} readings{"firstDate" in item?` · ${item.firstDate}${item.lastDate!==item.firstDate?` to ${item.lastDate}`:""}`:" · Removed from the dashboard"}</p>
