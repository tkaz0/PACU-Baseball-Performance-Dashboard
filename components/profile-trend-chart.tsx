...
export function ProfileTrendChart({ series }: { series: ProfileTrend[] }) {
  ...
  const id = useId();
  const trend = series.find(item => item.key === selected) ?? series[0];
  ...
  const format = (value: number) => trend.key === "height" ? formatHeight(value, trend.unit) : `${Number(value.toFixed(2))} ${trend.unit}`;
  return <section className="rounded-xl border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-4 sm:p-5" aria-label="Testing progress chart">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="m-0 text-lg font-bold">Testing Progress</h2><p className="muted mb-0 mt-1 text-xs">{trend.period} · {trend.points.length} test dates · {trend.source}</p></div><div><label className="sr-only" htmlFor={id}>Chart measurement</label><select id={id} className="max-w-full text-sm" value={trend.key} onChange={e => setSelected(e.target.value)}>{series.map(item => <option key={item.key} value={item.key}>{item.label}</option>)}</select></div></div>
    ...
    <svg viewBox="0 0 430 210" className="mt-2 block w-full" style={{ maxHeight: 250 }} role="img" aria-labelledby={`${id}-title ${id}-desc`}>
      <title id={`${id}-title`}>{trend.label} by test date</title><desc id={`${id}-desc`}>{trend.points.length} recorded tests from {first.date} to {last.date}. {format(first.value)} to {format(last.value)}. Vertical scale is zoomed to the recorded range. Exact values follow in chart data.</desc>
      {[min, (min + max) / 2, max].map(tick => <g key={tick}><line x1="55" x2="405" y1={y(tick)} y2={y(tick)} stroke="var(--line-subtle)"/><text x="47" y={y(tick) + 4} textAnchor="end" fill="var(--text-secondary)" fontSize="15">{Number(tick.toFixed(1))}</text></g>)}
