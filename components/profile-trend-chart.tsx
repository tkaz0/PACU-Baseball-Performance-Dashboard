...
export function ProfileTrendChart({ series }: { series: ProfileTrend[] }) {
  ...
  const format = (value: number) => trend.key === "height" ? formatHeight(value, trend.unit) : `${formatSourceNumber(value, trend.source, String(Number(value.toFixed(2))))} ${trend.unit}`;
  return <section className="rounded-xl border border-[var(--line-subtle)] bg-[var(--surface-panel)] p-4 sm:p-5" aria-label="Testing progress chart">
    ...
    <svg viewBox="0 0 430 210" className="mt-2 block w-full" style={{ maxHeight: 250 }} role="img" aria-labelledby={`${id}-title ${id}-desc`}>
      ...
      {[min, (min + max) / 2, max].map(tick => <g key={tick}><line x1="55" x2="405" y1={y(tick)} y2={y(tick)} stroke="var(--line-subtle)"/><text x="47" y={y(tick) + 4} textAnchor="end" fill="var(--text-secondary)" fontSize="15">{formatSourceNumber(tick, trend.source, String(Number(tick.toFixed(1))))}</text></g>)}
