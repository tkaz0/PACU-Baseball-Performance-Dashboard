...
export function PitchAssignmentReview({ session, ranges, fileHash, store, search, spinUnit, rpmConfirmed }: { session: FullSwingSession; ranges: PitchRange[]; fileHash?: string; store?: PitchAssignmentStore; search: string; spinUnit: string; rpmConfirmed: boolean }) {
  ...
  const pitchers=[...new Set(session.pitches.map(p=>p.identity))];
  const [selectedPitcher,setSelectedPitcher]=useState(pitchers[0]??"");
  ...
  const suggestions=rpmConfirmed?suggestPitchTypes(session.pitches,guidelines):[], suggested=new Map(suggestions.map(s=>[s.sourceRow,s]));
  ...
  const visible=session.pitches.filter(p=>p.identity.toLowerCase().includes(search.trim().toLowerCase()));
  ...
  return <section className="mt-5 space-y-4 border-t border-[var(--line-subtle)] pt-5" aria-label="Assign pitch types">
    ...
    <fieldset disabled={!ready || loading || saving} className="min-w-0 space-y-4">
      ...
      <p className="muted text-sm">{assignments.length} of {session.pitches.length} pitches assigned · {session.pitches.length-assignments.length} unassigned</p>
