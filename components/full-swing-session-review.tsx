...
export function FullSwingSessionReview({ session, fileHash, assignmentStore, includedIdentities, resultContext, saveResults, onResultsSaved }: { session: FullSwingSession; resultContext?: PitchResultContext; saveResults?: SaveImportAction; includedIdentities?: string[]; fileHash?: string; assignmentStore?: PitchAssignmentStore; onResultsSaved?: () => void }) {
  ...
  return <section className="my-6 space-y-5 rounded-xl border border-[var(--line-subtle)] p-4 sm:p-5" aria-label="Full Swing session review">
    <div><h3 className="m-0 text-lg font-bold">Roster Player Summaries</h3><p className="muted mb-0 text-sm">{session.date} · {session.eventCount} {session.mode === "Machine BP" ? "swings" : "pitches"} in the source file. Only selected roster players appear below; — means no recorded value.</p></div>
