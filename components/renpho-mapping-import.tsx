...
export function RenphoMappingImport({ roster }: { roster: { id?: string; code: string; name: string }[] }) {
  ...
  return <section className="panel space-y-5 p-5 sm:p-7">
    ...
    {!!rows.length && <>
      ...
      <div className="table-wrap"><table><caption className="sr-only">Roster RENPHO ID matches</caption><thead><tr><th>Player</th><th>PAC ID</th><th>RENPHO ID</th></tr></thead><tbody>{rows.map(row => <tr key={row.renpho_id}><th scope="row">{(() => { const player = roster.find(athlete => athlete.code === row.athlete_code); return player?.id ? <Link prefetch={false} href={`/athletes/${player.id}`} className="underline underline-offset-2">{player.name}</Link> : player?.name; })()}</th><td>{row.athlete_code}</td><td>{row.renpho_id}</td></tr>)}</tbody></table></div>
