...
export function Sidebar({ roles, athleteId, isPreview = false }: { roles: Role[]; athleteId: string | null; isPreview?: boolean }) {
  ...
  const links = [
    ...
    ...(staff && (!isPreview || roles.includes("coach")) ? [{ href: "/team-progress", label: "Team Progress", icon: TrendingUp, group: "Staff Tools" }, { href: "/compare", label: "Compare Players", icon: ArrowLeftRight, group: "Staff Tools" }, { href: "/analytics", label: "Analytics", icon: ChartScatter, group: "Staff Tools" }, { href: "/testing/changes", label: "What Changed", icon: BellDot, group: "Staff Tools" }, { href: "/game-stats/review", label: "Data Review", icon: ShieldCheck, group: "Staff Tools" }, { href: "/testing/coverage", label: "Testing", icon: ClipboardList, group: "Staff Tools" }, { href: "/imports", label: "Import Center", icon: Upload, group: "Staff Tools" }] : []),
