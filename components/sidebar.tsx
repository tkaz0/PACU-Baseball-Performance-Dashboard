...
export function Sidebar({ roles, athleteId, isPreview = false }: { roles: Role[]; athleteId: string | null; isPreview?: boolean }) {
  ...
  const links = [
    ...
    ...(staff && (!isPreview || roles.includes("coach")) ? [{ href: "/analytics", label: "Analytics", icon: ChartScatter, group: "Staff Tools" }, { href: "/game-stats/log", label: "Game Log", icon: ClipboardList, group: "Staff Tools" }, { href: "/game-stats/review", label: "Data Review", icon: ShieldCheck, group: "Staff Tools" }, { href: "/testing", label: "Testing", icon: ClipboardList, group: "Staff Tools" }, { href: "/imports", label: "Information Imports", icon: Upload, group: "Staff Tools" }] : []),
