...
import { UsersRound, Upload, ShieldCheck, UserRound, Settings, BarChart3, ListOrdered, ClipboardList } from "lucide-react";
...
export function Sidebar({ roles, athleteId, isPreview = false }: { roles: Role[]; athleteId: string | null; isPreview?: boolean }) {
  ...
  const links = [
    ...
    ...(staff && (!isPreview || roles.includes("coach")) ? [{ href: "/testing", label: "Testing", icon: ClipboardList, group: "Staff Tools" }, { href: "/imports", label: "Information Imports", icon: Upload, group: "Staff Tools" }] : []),
