...
export function LeaderboardBoard({ group, panels }: { group: LeaderboardGroup; panels: LeaderboardPanel[] }) {
  ...
  return <>
    ...
    {populated.length > 0 ? <div className="columns-1 gap-5 xl:columns-2">{populated.map(({ comparison, rows }) => <div key={comparison.metricKey} className="mb-5 break-inside-avoid"><LeaderboardResults rows={rows} metric={PLAYER_METRICS.find(metric => metric.key === comparison.metricKey)!} unit={comparison.unit} source={comparison.source} /></div>)}</div>
