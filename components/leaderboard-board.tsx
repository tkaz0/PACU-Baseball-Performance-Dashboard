...
export function LeaderboardBoard({ group, panels, session = "in_game", pitches = [], selectedPitch }: { group: LeaderboardGroup; panels: LeaderboardPanel[]; session?: LeaderboardSession; pitches?: readonly string[]; selectedPitch?: string }) {
  ...
  return <>
    ...
    {populated.length > 0 && <details className="mt-6 text-xs text-[var(--text-secondary)]"><summary className="w-fit cursor-pointer font-medium">About These Rankings</summary><div className="mt-3 max-w-3xl space-y-2 leading-relaxed"><p>Each board uses a player’s latest result from the same test, units, and time period. Fall 2026 comes first. Earlier body tests appear only when a Fall result is not available. Test dates sit beside the results.</p><p>If a stat has results from several devices or test types, the board with the most players is shown. Different tests and units stay separate, as do In-Game and Practice. Pick a pitch to see its speed and spin rankings; four-seamers and other fastball types keep their own boards. Pitch averages and highs come from each player’s latest matching session. Height appears in feet and inches.</p><p>Height ranks taller players first; muscle mass ranks higher totals first; body fat ranks lower percentages first. Ties share a place, such as 1, 1, 3. The colored bar shows a result against the top number on that board, not a percentile. Body and spin numbers are comparisons, not health grades or pitch-quality grades.</p></div></details>}
  </>;
}
