...
export function LeaderboardBoard({ group, panels }: { group: LeaderboardGroup; panels: LeaderboardPanel[] }) {
  ...
  return <>
    ...
    {populated.length > 0 ? <div className="leaderboard-grid">{populated.map(({ comparison, rows }) => <LeaderboardResults key={comparison.metricKey} rows={rows} metric={PLAYER_METRICS.find(metric => metric.key === comparison.metricKey)!} unit={comparison.unit} source={comparison.source} />)}</div>
