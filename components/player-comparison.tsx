...
export function PlayerComparison({data,today}:{data:CoachingData;today:string}){
 ...
 const choices=sharedSources.map(source=>({id:source==="qpa_fall_2026"?"":PITCHING_CUMULATIVE,label:source==="qpa_fall_2026"?"Hitting · Fall to Date":"Pitching · Fall to Date"}));
 ...
 return <div className={styles.tools}>
  ...
  <div className={styles.comparisonList}><div className={styles.compareHead}><span>{first.name}</span><span>Metric</span><span>{second.name}</span></div>
   {tests.map(row=>{const direction=PLAYER_METRICS.find(m=>m.key===row.metric)?.direction??"neutral";return <ComparisonRow key={row.key} label={row.label} metric={row.metric} context={row.source} first={first.name} second={second.name} lead={comparisonLead(row.first?.value,row.second?.value,direction,row.comparable)} note={!row.comparable?(row.gap!==null?`Tests ${row.gap} days apart`:"Waiting for matching tests"):direction==="neutral"?"Measured values":direction==="higher"?"Higher leads":"Lower leads"} a={<><strong>{row.first?coachingValue(row.first.value,row.metric,row.unit,row.source):"—"}</strong><p className={styles.meta}>{row.first?leaderboardTestDate(row.first.date):!row.eligibleA?"Not applicable":row.reviewA?"Needs review":"Not yet tested"}</p></>} b={<><strong>{row.second?coachingValue(row.second.value,row.metric,row.unit,row.source):"—"}</strong><p className={styles.meta}>{row.second?leaderboardTestDate(row.second.date):!row.eligibleB?"Not applicable":row.reviewB?"Needs review":"Not yet tested"}</p></>}/>;})}
   {games.map(row=>{const direction=row.first?.direction??row.second?.direction??"neutral";const render=(r:typeof row.first)=><><strong>{r?coachingValue(r.value,r.metric,r.unit):"—"}</strong>{r?<><GameOpportunity source={r.source} metric={r.metric} count={r.opportunities}/><p className={styles.meta}>Updated {leaderboardTestDate(new Intl.DateTimeFormat("en-CA",{timeZone:"America/Los_Angeles",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(r.updatedAt)))}</p></>:<p className={styles.meta}>No saved result</p>}</>;return <ComparisonRow key={row.metric} label={row.label} metric={row.metric} context={selectedEvent?pitchingPeriodLabel(selectedEvent,row.first?.playedOn??row.second?.playedOn??null):"QPA · Cumulative"} first={first.name} second={second.name} lead={comparisonLead(row.first?.value,row.second?.value,direction,row.comparable)} note={!row.comparable?"Both players need results from the same Fall update":direction==="neutral"?"Recorded values":direction==="higher"?"Higher leads":"Lower leads"} a={render(row.first)} b={render(row.second)}/>;})}
  </div>
  {!tests.length&&!games.length&&<section className={`${styles.panel} ${styles.empty}`}>{category==="Game Stats"&&!choices.length?"Choose two hitters or two pitchers to compare their Fall game stats.":`No recorded ${category.toLowerCase()} results for this comparison yet.`}</section>}
  <p className={styles.meta}>{category==="Game Stats"?"Check the chances behind each rate. Counting stats depend on playing time; pitching adds the recorded Fall weeks or games.":"Players are compared on the same test and units within the selected dates. Body numbers do not declare a winner."}</p></>:<p className={styles.notice}>{first&&second?"Choose two different players to compare.":"Choose two players from the roster above."}</p>}
 </div>;
}
