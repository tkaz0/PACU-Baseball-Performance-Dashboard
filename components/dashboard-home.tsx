...
function GameSnapshot({summary,kind}:{summary:TeamGameSummary;kind:"Hitting"|"Pitching"}) {
  ...
  return <section className={styles.gameCard} aria-label={`${kind} game snapshot`}>
    ...
    {!summary.entries?<p className={styles.empty}>Fall game stats will appear after coaches add the next sheet update.</p>:<dl className={styles.gameNumbers}>{metrics.map(m=><div key={m.metric}><dt>{m.label}<StatInfo metric={m.metric} label={m.label}/></dt><dd>{formatTeamGameMetric(m)}</dd><p>{m.pending?"Counts need review":m.opportunities===undefined?"No chances recorded yet":m.opportunityLabel==="outs"?`${formatInnings(m.opportunities)} IP`:`${m.opportunities} ${m.opportunityLabel}`}</p></div>)}</dl>}
...
function CoachThisWeek({digest,dueFocus,reviewCount}:{digest:ReturnType<typeof coachUpdateDigest>;dueFocus:DueCoachFocus[];reviewCount:number}) {
  const recent=digest.updatedPlayers.slice(0,3), followUps=digest.stale.slice(0,3);
  return <section className={styles.weekSection} aria-label="This week for coaches">
    <div className={styles.blockHeading}><div><p className={styles.kicker}>Coaching Desk</p><h2>This Week</h2><p>New results and testing follow-ups in one place.</p></div><Link prefetch={false} href="/testing/changes" className={styles.panelLink}>All Changes<ArrowRight size={15}/></Link></div>
    <div className={styles.weekGrid}>
      <article className={styles.weekCard}><span className={styles.weekCount}>{digest.updatedPlayers.length}</span><h3>Players Updated</h3><p>Results saved in the past seven days · {digest.changes.length} measured changes</p>{recent.length?<ul>{recent.map(player=><li key={player.id}><Link prefetch={false} href={`/athletes/${player.id}`}>{player.name}</Link><small>{player.areas.join(" · ")}</small></li>)}</ul>:<p className={styles.weekEmpty}>No new player results this week.</p>}<Link prefetch={false} href="/testing/changes">Review updates <ArrowRight size={13}/></Link></article>
      <article className={styles.weekCard}><span className={styles.weekCount}>{dueFocus.length}</span><h3>Retests Due</h3><p>Coach focus dates overdue or due within seven days · {digest.stale.length} longer testing gaps</p>{dueFocus.length?<ul>{dueFocus.slice(0,3).map(item=><li key={item.id}><Link prefetch={false} href={`/athletes/${item.athleteId}`}>{item.playerName}</Link><small>{item.title} · {date(item.targetDate)}</small></li>)}</ul>:followUps.length?<ul>{followUps.map(player=><li key={player.id}><Link prefetch={false} href={`/athletes/${player.id}`}>{player.name}</Link><small>{player.lastDate?`Last test ${date(player.lastDate)}`:"No Fall test yet"}</small></li>)}</ul>:<p className={styles.weekEmpty}>No scheduled retests or longer testing gaps.</p>}<Link prefetch={false} href="/testing/coverage">Open testing checklist <ArrowRight size={13}/></Link></article>
      <article className={styles.weekCard}><span className={styles.weekCount}>{reviewCount}</span><h3>Game-Stat Review</h3><p>Team rates held for source-count review</p><p className={styles.weekEmpty}>{reviewCount?"Check the source counts before using these rates.":"No team rates currently flagged."}</p><Link prefetch={false} href="/game-stats/review">Review game stats <ArrowRight size={13}/></Link><Link prefetch={false} href="/imports">Open import center <ArrowRight size={13}/></Link></article>
    </div>
  </section>;
}
    ...
    {!summary?<section className={styles.panel}><h2>Your profile is being connected</h2><p className={styles.empty}>Your administrator will link your account to the correct player profile. Your results will appear here once it is connected.</p></section>:<>
      ...
      <div className={styles.mainGrid}>
        ...
        <section className={styles.panel} aria-label="Recent data updates">
          ...
          {staff&&<div className={styles.nextStep}><ClipboardCheck size={21}/><div><strong>Ready for the next report?</strong><p>Check player names and test dates before adding new results.</p></div></div>}
