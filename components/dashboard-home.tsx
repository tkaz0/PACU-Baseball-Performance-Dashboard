...
export function DashboardHome({staff,athleteId,summary,leaderboards=[],dueFocus=[]}:{staff:boolean;athleteId:string|null;summary:(HomeSummary & {coachDigest?:ReturnType<typeof coachUpdateDigest>})|null;leaderboards?:HomeLeaderboard[];dueFocus?:DueCoachFocus[]}) {
  ...
  return <div className={styles.home}>
    <section className={styles.welcome} aria-label="Dashboard home">
      <div><p className={styles.kicker}>Pacific University <span>/</span> Boxer Baseball</p><h1>{staff?"The Team, at a Glance.":"Your Baseball Home."}</h1><p className={styles.intro}>{staff?"Results, rankings and coaching follow-ups for Fall ball.":"Your latest results and team rankings, all in one place."}</p>
    ...
    {summary&&<div className={styles.pulseStrip} aria-label="Fall dashboard snapshot"><div><span>{staff?"Players with Results":"Areas with Results"}</span><strong>{staff?summary.playersWithResults:savedAreas}<small> / {staff?summary.players:4}</small></strong></div><div><span>Latest Update</span><strong className={styles.pulseDate}>{latestUpdate?date(latestUpdate.date,true):"Awaiting results"}</strong>{latestUpdate&&<small className={styles.pulseDetail}>{latestUpdate.label}</small>}</div><div><span>Game Stats Updated</span><strong className={styles.pulseDate}>{latestGameUpdate?date(latestGameUpdate,true):"Awaiting results"}</strong></div></div>}
