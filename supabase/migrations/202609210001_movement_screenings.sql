begin;
create table public.movement_screenings (
 id uuid primary key default gen_random_uuid(),
 athlete_id uuid not null references public.athletes(id) on delete restrict,
 source_sheet_id integer not null check(source_sheet_id>=0),
 screened_on date not null check(screened_on between date '2026-09-01' and date '2026-12-31'),
 source_hash text not null check(source_hash ~ '^[a-f0-9]{64}$'),
 readings jsonb not null,
 created_by uuid not null references auth.users(id),
 created_at timestamptz not null default now(),
 unique(source_sheet_id,screened_on)
);
alter table public.movement_screenings enable row level security;
revoke all on public.movement_screenings from public,anon,authenticated;
grant select on public.movement_screenings to authenticated;
create policy movement_own_or_staff on public.movement_screenings for select to authenticated using(private.can_read_athlete(athlete_id));
create function private.import_movement_screenings(p_payload jsonb) returns jsonb language plpgsql security definer set search_path='' as $$
 ...
 for report in select value from jsonb_array_elements(p_payload->'reports') loop
  ...
  for idx in 0..23 loop
   ...
   if v is not null then
    if row_number between 4 and 19 then
...
$$;
revoke all on function private.import_movement_screenings(jsonb) from public,anon,authenticated;
grant execute on function private.import_movement_screenings(jsonb) to authenticated;
create function public.staff_import_movement_screenings(p_payload jsonb) returns jsonb language sql security invoker set search_path='' as $$select private.import_movement_screenings(p_payload);$$;
revoke all on function public.staff_import_movement_screenings(jsonb) from public,anon,authenticated;
grant execute on function public.staff_import_movement_screenings(jsonb) to authenticated;
commit;
