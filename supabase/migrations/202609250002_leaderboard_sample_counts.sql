-- Expose only reviewed numeric sample counts in the existing signed-in leaderboard projection.
-- Full Swing summary swing counts were not saved historically and remain unknown.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('rank',l.place,'athleteCode',l.athlete_code,'name',l.display_name,
    ...
    'sampleCount',case when samples.value>0 and samples.value=trunc(samples.value) then samples.value::integer
    ...
    'sampleUnit',case when samples.value>0 and samples.value=trunc(samples.value) then
  ...
  left join lateral (
    select c.value from public.performance_measurements c
    where c.athlete_id=l.athlete_id and c.file_hash=selected.file_hash and c.measured_at=l.measured_at
      ...
      and c.unit='count'
    order by c.observation_id limit 1
  ) samples on true
  left join lateral (
